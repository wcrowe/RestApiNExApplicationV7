...
using RestApiNExApiV6.Api;
using RestApiNExApiV6.Api.Middlewares;
...
/// <summary>
/// Designed by AnaSoft Inc.(c) 2019-2022
/// http://www.anasoft.net/apincore
/// 
/// NOTE: Tests are not working with InMemory database.
///       Must update database connection in appsettings.json - "RestApiNExApiV6DB".  
...
namespace RestApiNExApiV6.Test
{
    ...
    #region "Build Test web site from 

    /// <summary>
    /// Setup testing web host factory
    /// </summary>
    /// <typeparam name="TEntryPoint"></typeparam>
    public class TestingRESTApiFactory<TEntryPoint> : WebApplicationFactory<Program> where TEntryPoint : Program
    {
    ...
    public class HttpClientFixture : IDisposable
    {
        ...
        {
            Client = (new TestingRESTApiFactory<Program>()).CreateClient();
        }
        public void Dispose()
        {
            Client.Dispose();
    ...
    public class TokenTest : BaseTest
    {
        ...
        public static async Task token_get(HttpClient client)
        {
            ...
            { //IS4
                ...
                //var response1 = await tokenClient1.RequestClientCredentialsAsync("RestApiNExApiV6");
                //var resp1 = response1.Json;


                //BAD client test
                var response = LoginUsingIdentityServer(is4token, "RestApiNExApiV6-BAD", "secret", "read-write", "my@email.com", "mysecretpassword123");
                ...
                //BAD grant test
                response = LoginUsingIdentityServer(is4token, "RestApiNExApiV6Client", "secret", "read-write", "my@email.com", "mysecretpassword123-BAD");
                ...
                response = LoginUsingIdentityServer(is4token, "RestApiNExApiV6Client", "secret", "read-write", "my@email.com", "mysecretpassword123");
                response_json = response.AccessToken;
                if (response.IsError)
                {
                    Console.WriteLine(response.Error);
    ...
    public class UserTest : BaseTest
    {
        ...
        public async Task user_getbyid()
        {
            ...
            // lazy-loading test
            if (configuration["ConnectionStrings:UseLazyLoadingProxies"] == "True" && configuration["RespositoryUseThreadSafeDictionary"] == "False")
            {
    ...
    public class ZLoadTest : BaseTest
    {
        ...
        public async Task LoadTest()
        {
            ...
            while (i < loopmax)
            {
                ...
                await util.GetAccountAsync(client, accountId);
                await util.GetUserAsync(client, userId);
                await util.removeUser(client, userId);
                await util.removeAccount(client, accountId);
                i++;
            }
