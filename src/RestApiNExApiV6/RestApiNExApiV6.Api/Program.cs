...
using RestApiNExApiV6.Api.Settings;
using RestApiNExApiV6.Api.Utilities;
using RestApiNExApiV6.Domain.Mapping;
...
try
{
    ...
    string connectionString = "ConnectionStrings:RestApiNExApiV6DB";
    ...
    else
    {
        ...
        .AddIdentityServerAuthentication(option =>
        {
            ...
            option.ApiName = "RestApiNExApiV6";  //This is the resourceAPI that we defined in the Config.cs in the AuthServ project (apiresouces.json and clients.json). They have to be named equal.
        });
...
.AddNewtonsoftJson(options => { options.SerializerSettings.ReferenceLoopHandling = Newtonsoft.Json.ReferenceLoopHandling.Ignore; });  //NO entity classes' children serialization
    ...
    {
        c.SwaggerDoc("v1", new OpenApiInfo { Title = "RestApiNExApiV6 API", Version = "v1" });
        c.SwaggerDoc("v2", new OpenApiInfo { Title = "RestApiNExApiV6 API", Version = "v2" });
...
app.UseSwaggerUI(c =>
{
    c.SwaggerEndpoint("/swagger/v1/swagger.json", "RestApiNExApiV6 API V1");
    c.SwaggerEndpoint("/swagger/v2/swagger.json", "RestApiNExApiV6 API V2");
