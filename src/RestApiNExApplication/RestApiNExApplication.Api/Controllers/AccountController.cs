...
namespace RestApiNExApplication.Api.Controllers
{
    [ApiVersion("1.0")]
    ...
    public class AccountController : ControllerBase
    {
        ...
        public AccountController(AccountService<AccountViewModel, Account> accountService, IAppCache cache)
        {
        ...
        public IEnumerable<AccountViewModel> GetAll()
        {
            ...
            var items = _lazyCache.GetOrAdd($"Accounts", () => _accountService.GetAll());
            return items;
        }
        ...
        public IEnumerable<AccountViewModel> GetAll(int page, int quantityperpage)
        {
            Pagination pagination = new Pagination { Page = page, QuantityPerPage = quantityperpage };
            var items = _accountService.GetAll(pagination);
            HttpContext.Response.Headers.Add("pagesQuantity", _accountService.PaginationPagesCnt.ToString());
            return items;
        }
        ...
        {
            var items = _lazyCache.GetOrAdd($"Accounts-{name}", () => _accountService.Get(a => a.IsActive && a.Name == name));
            return Ok(items);
        }
        ...
        {
            var item = _lazyCache.GetOrAdd($"Account-{id}", () => _accountService.GetOne(id));
            if (item == null)
            {
